...
Implements TARA-k (KS-based) and TARA-m (Martingale-based) detectors
for conformal prediction-based quantum anomaly detection.
...
class TARAk:
    ...
    Under H0 (exchangeable data), p-values should be uniform.
    """
    ...
    def test(self, data: Dict[str, np.ndarray],
        ...
        Returns:
            Dict with 'ks_statistic', 'ks_pvalue', 'detected', 'p_values'
        """
...
class TARAm:
    ...
    def _compute_bet(self, pv: float) -> float:
        ...
        - linear: bet = 1 + epsilon(1 - 2p)  [grows for p < 0.5, shrinks for p > 0.5]
        - jumper: bet = (1-epsilon) * p^(-epsilon)  [grows for small p]
        - twosided: bet = 1 + epsilon * |0.5 - p| * 2  [grows for any deviation from 0.5]
...
class TARAm_TwoSided(TARAm):
    ...
    This version catches that pattern.
    """

    def __init__(self, calibration_data: Dict[str, np.ndarray],
                 epsilon: float = 0.5):
        super().__init__(calibration_data, epsilon, betting='twosided')
